...
export interface RuntimeAlgorithm {
  consume(key: string, now: number, signal?: AbortSignal): Promise<AlgorithmConsumeResult>;

  peek(key: string, now: number, signal?: AbortSignal): Promise<AlgorithmConsumeResult>;
}
